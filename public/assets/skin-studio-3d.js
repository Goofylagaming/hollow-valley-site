...
const MODEL_URL = "https://cdn.3dassets.dev/assets/4895/v1/model.glb";
...
camera.position.set(7.6, 3.1, 8.8);
...
const key = new THREE.DirectionalLight(0xffefd3, 3.25);
key.position.set(5, 8, 7);
key.castShadow = true;
key.shadow.mapSize.set(1024, 1024);
scene.add(key);

const rim = new THREE.DirectionalLight(0x5cffad, 1.45);
rim.position.set(-7, 4, -5);
scene.add(rim);

const fill = new THREE.DirectionalLight(0x7fa8bf, 0.75);
...
function frameModel(root) {
  ...
  ground.position.y = finalBox.min.y - 0.03;

  const target = new THREE.Vector3(0, finalBox.min.y + finalSize.y * 0.49, 0);
  const distance = Math.max(7.5, finalSize.length() * 0.82);
  camera.position.set(distance * 0.78, target.y + finalSize.y * 0.22, distance);
...
new GLTFLoader().load(
  ...
  (gltf) => {
    ...
    setLoaded(true, "Tyrannosaurus · interactive 3D preview");
  },
  (event) => {
