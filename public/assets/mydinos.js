...
function renderDinoCard(dino) {
  ...
  return `
    <article class="dino-card-v2 storage-dino-card" data-prime="${dino.isPrime ? "true" : "false"}" data-slot="${escapeHtml(dino.slot)}" data-species="${escapeHtml(dino.species || "")}">
...
function openSellPanel(dino) {
  const growth = growthPct(dino);
  const { content } = openDialogWith(
    "Sell parked dino",
    `List your ${growth}% ${dino.species} for Valley Coin. While listed it is moved into marketplace escrow and cannot be redeemed or edited.`,
    `<form id="sell-dino-form" class="parked-tool-form">
      <label><span>SALE PRICE · VALLEY COIN</span><input name="price" type="number" min="1" step="1" max="100000000" required></label>
      <div class="parked-tool-note warning">Listing moves the real DinoStorage file into escrow. Cancelling returns it; selling transfers it into the buyer’s My Dinos storage.</div>
      <button class="primary-button" type="submit">List for sale <b>→</b></button>
    </form>`
  );
  content.querySelector("#sell-dino-form").addEventListener("submit", async (event) => {
    event.preventDefault();
    const form = event.currentTarget;
    const price = Number(new FormData(form).get("price"));
    if (!Number.isSafeInteger(price) || price <= 0) return alert("Enter a valid whole-number Valley Coin price.");
    if (!confirm(`List this ${dino.species} for ${price.toLocaleString()} Valley Coin? It will move into marketplace escrow.`)) return;
    const button = form.querySelector('button[type="submit"]');
    button.disabled = true;
    button.textContent = "Moving to escrow…";
    try {
      await api("/api/marketplace/listings", { method: "POST", body: JSON.stringify({ slot: dino.slot, price }) });
      alert("Dino listed. It is now locked in marketplace escrow.");
      ensureParkedToolsDialog().close();
      await refresh();
    } catch (error) {
      alert(error.message || "Could not list dino.");
      button.disabled = false;
...
function wireParkedTools(grid) {
  grid.querySelectorAll(".parked-tool").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (button.dataset.tool === "sell") openSellPanel(dino);
    });
  });
}
