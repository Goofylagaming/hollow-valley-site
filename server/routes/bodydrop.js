  ...
  if (Number.isInteger(error?.status)) {
    return {
      ...
      body: {
        ...
        ...(Array.isArray(error.payload?.allowedDropTypes) ? { allowedDropTypes: error.payload.allowedDropTypes } : {}),
...
router.get("/", requireAuth, async (req, res) => {
  if (!req.user.steam_id) {
    return res.json({
      ...
      restrictions: { carnivoreOnly: true, maxGrowthPercent: 60, dietValidated: true },
  ...
  try {
    ...
    return res.json({
      ...
      dietEligibility: result.dietEligibility || { eligible: false, configured: false, reason: null },
      restrictions: result.restrictions || { carnivoreOnly: true, maxGrowthPercent: 60, dietValidated: true },
